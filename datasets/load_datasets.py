...
root= '/fastdata/ksingh/robust/transfer_datasets'

def build_dataset(root, transform, is_train):
...
def load_data(dname, is_clip=False):
    transform = get_transform(is_clip)
    if dname == 'cars':
        trainval_dataset = torchvision.datasets.StanfordCars(root=f'{root}', split='train', transform=transform, download=True)
        test_dataset = torchvision.datasets.StanfordCars(root=f'{root}', split='test', transform=transform, download=True)

    if dname == 'cifar10':
        trainval_dataset = torchvision.datasets.CIFAR10(root=f'{root}', train=True, transform=transform, download=True)
        test_dataset = torchvision.datasets.CIFAR10(root=f'{root}', train=False, transform=transform, download=True)

    if dname == 'cifar100':
        trainval_dataset = torchvision.datasets.CIFAR100(root=f'{root}', train=True, transform=transform)
        test_dataset = torchvision.datasets.CIFAR100(root=f'{root}', train=False, transform=transform)

    if dname == 'dtd':
        train_dataset = torchvision.datasets.DTD(root=f'{root}', split='train', transform=transform, download=True)
        val_dataset = torchvision.datasets.DTD(root=f'{root}', split='train', transform=transform)
        trainval_dataset = torch.utils.data.ConcatDataset([train_dataset, val_dataset])
        test_dataset = torchvision.datasets.DTD(root=f'{root}', split='test', transform=transform, download=True)

    if dname == 'pets':
        trainval_dataset = torchvision.datasets.OxfordIIITPet(root=f'{root}', split='trainval', transform=transform, download=True)
        test_dataset = torchvision.datasets.OxfordIIITPet(root=f'{root}', split='test', transform=transform, download=True)

    if dname == 'sun397':
        trainval_dataset = torchvision.datasets.SUN397(root=f'{root}', transform=transform, download=True)
        generator = torch.Generator().manual_seed(42)
        import ipdb; ipdb.set_trace()
        trainval_dataset, test_dataset = random_split(trainval_dataset, [19850, 19850], generator=generator)

    if dname == 'food101':
        trainval_dataset = torchvision.datasets.Food101(root=f'{root}', split='train', transform=transform, download=True)
        test_dataset = torchvision.datasets.Food101(root=f'{root}', split='test', transform=transform, download=True)

    if dname == 'flowers102':
        train_dataset = torchvision.datasets.Flowers102(root=f'{root}', split='train', transform=transform, download=True)
        val_dataset = torchvision.datasets.Flowers102(root=f'{root}', split='val', transform=transform, download=True)
        trainval_dataset = torch.utils.data.ConcatDataset([train_dataset, val_dataset])
        test_dataset = torchvision.datasets.Flowers102(root=f'{root}', split='test', transform=transform)
    
    if dname == 'fgcv-aircraft':
        trainval_dataset = torchvision.datasets.FGVCAircraft(root=f'{root}', split='trainval', transform=transform, download=True)
        test_dataset = torchvision.datasets.FGVCAircraft(root=f'{root}', split='test', transform=transform, download=True)
    ...
    if dname == 'imagenet':
        ...
        trainval_dataset = build_dataset(root='/fastdata/ksingh/robust/ILSVRC2012', is_train=True, transform=train_transform)
        test_dataset = build_dataset(root='/fastdata/ksingh/robust/ILSVRC2012', is_train=False, transform=val_transform)
    ...
    if dname == 'imagenet-r':
        ...
        imagenet_r_dataset  = datasets.ImageFolder(root="/fastdata/ksingh/robust/calibration/imagenet-r/", transform=test_transform)
        print('Using imagenet R, second args is mask')
        return imagenet_r_mask, imagenet_r_dataset
    ...
    if dname == 'imagenet-a':
        ...
        imagenet_a_dataset = datasets.ImageFolder(root="/fastdata/ksingh/robust/calibration/imagenet-a/", transform=test_transform)
        return indices_in_1k, imagenet_a_dataset
