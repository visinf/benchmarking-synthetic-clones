...
sys.path.append('/visinf/home/ksingh/syn-rep-learn/')
sys.path.append('/visinf/home/ksingh/syn-rep-learn/third_party/linear_probe_clip/')
...
def main(args):
    ...
    dataset = load_data(config.exp.dataset_name)
    train_dset, test_dset = load_data(config.exp.dataset)
    ...
    train_loader = DataLoader(
        ...
        pin_memory=False,
        transforms=Compose([
            Resize(max(config.exp.input_size), interpolation=interp_mode),
            CenterCrop(config.exp.input_size),
            ToTensor()
        ])
    )
    ...
    #   Setup Network
    ########################################
    clip_model, _ = clip.load("ViT-B/16", "cuda", jit=False)
    clip_model.eval()
    ###################################################################################################################
