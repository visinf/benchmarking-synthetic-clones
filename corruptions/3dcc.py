...
#                    'flash', 'fog_3d', 'h265_crf',
#                    'low_light', 'xy_motion_blur', 'z_motion_blur'] # 12 corruptions in ImageNet-3DCC
corruptions_3dcc = ['near_focus', 'far_focus', 'fog_3d', 'xy_motion_blur', 'z_motion_blur'] # 12 corruptions in ImageNet-3DCC
...
def main(config):
    ...
    for corruption in corruptions_3dcc:
        for s in [1, 2, 3, 4, 5]:  # 5 severity levels
            data_dir = f'/fastdata/ksingh/robust/ImageNet-3DCC/{corruption}/{s}/'
    ...
    if config.exp.ablation:
        config.exp.save_path = f'{config.exp.save_path}/3DCC/ablations/'
        ...
        report_json(result_dict, f'{config.exp.save_path}/3DCC/', fn=f'{config.exp.model}.json')
